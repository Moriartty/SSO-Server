...
app.all('*', (req, res, next) => {
    ...
    const whiteOriginDomain = '.test-teye.com'

    let origin = req.headers.origin
    if (origin) {
        (origin.indexOf('localhost')||origin.indexOf(whiteOriginDomain) > -1)
...
app.post('/login', async (req, res, next) => {
    ...
    try {
        ...
        res.setHeader(
            'Set-Cookie',
            `token=${newToken};domain=test-teye.com;max-age=${tokenConfig.expiresIn};`)
